...
/*==================================================== */
    ...
    function test_CreateDeal() public {
        ...
        (address p, address w, uint256 bal, uint256 tm, uint8 cm, Escrow.Status status) = escrow.deals(1);
        assertEq(p, payer);
        assertEq(w, payee);
        assertEq(tm, 2);
        assertEq(cm, 0);
        assertEq(bal, 2 ether);
        assertTrue(status == Escrow.Status.InProgress);
    }
    ...
    function test_completeDeal() public {
        ...
        (,, uint256 totalBalance,,, Escrow.Status status) = escrow.deals(dealId);
        assertEq(totalBalance, 0);
        assertTrue(status == Escrow.Status.Completed);
    }
    ...
    function test_raiseDispute() public {
        ...
        (,,,,, Escrow.Status status) = escrow.deals(dealId);
        assertTrue(status == Escrow.Status.Disputed);
    }
    ...
    function test_cancelDeal() public {
        ...
        (,, uint256 totalBalance,,, Escrow.Status status) = escrow.deals(dealId);
        assertEq(totalBalance, 0);
        assertTrue(status == Escrow.Status.Canceled);
    }
